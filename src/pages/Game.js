...
const Game = () => {
  ...
  let updateSquares = (actor) => {
    const squaresCopy = squares.slice();

    for (let i = 0; i < GameManager.BOARD_SIZE * GameManager.BOARD_SIZE; i++) {
      let target = { x: i % GameManager.BOARD_SIZE, y: Math.floor(i / GameManager.BOARD_SIZE) };
      if (actor !== null && actor.getX() === target.x && actor.getY() === target.y) {
        squaresCopy[i] = SquareColor.HIGHLIGHT;
      } else {
        squaresCopy[i] = SquareColor.OUT_OF_RANGE;
      }
    }
  ...
  const updateLevel = async () => {
    ...
    GameManager.getAIController().act((actor) => updateSquares(actor));
